...
public class Board {
    ...
    private int getScore(Word word) {
        ...
        for(int i=0;i<len;i++){
            switch (gameBoard[row][col].color){
                case 1:
                    sum+=word.getTile()[i].getScore()*2;
                    break;
                case 2:
                    sum+=word.getTile()[i].getScore()*3;
                    break;
                case 3:
                    sum+=word.getTile()[i].getScore();
                    mul*=2;
                    break;
                case 4:
                    sum+=word.getTile()[i].getScore();
    ...
    public ArrayList<Word> getWords(Word word) {
        ...
        for (int i = 0; i < word.getTile().length; i++) {
            if (word.getTile()[i] != null) {
                if(word.isVertical()){
                    if(i==0 && row>0 && gameBoard[row-1][col].tile!=null || i==word.getTile().length-1 && row<14 && gameBoard[row+1][col].tile!=null){
                        if(!flag)
                    ...
                    if(i==0 && col>0 && gameBoard[row][col-1].tile!=null || i==word.getTile().length-1 && col<14 && gameBoard[row][col+1].tile!=null){
                        if(!flag)
                            createdWords.add(getWord(row, col, word, i, false));
                        flag = true;
                    }
    ...
    private Word getWord(int row, int col, Word word, int i, boolean vertical){
        ...
        if(vertical){
            if(word.isVertical()) {
                ...
                endRow = word.getRow() + word.getTile().length - 1;
            }
            ...
            for(int j=0;j<tiles.length;j++){
                if(gameBoard[startRow+j][col].tile==null){
                    tiles[j] = word.getTile()[index];
                    index++;
                    while(index<word.getTile().length-1 && word.getTile()[index]==null){
                        index++;
                    }
                }
                else {
            ...
            if(!word.isVertical()){
                ...
                endCol = word.getCol() + word.getTile().length - 1;
            }
    ...
    private void wordPlacing(Word word){
        ...
        for(int i=0;i<len;i++){
            if(gameBoard[row][col].getTile()==null){
                gameBoard[row][col].setTile(word.getTile()[i]);
            }
            if(word.isVertical())
                row++;
            else{
    ...
    private boolean donotReplaceLetter(Word word) {
        if (word.isVertical()) {
            for (int i = 0; i < word.getTile().length; i++) {
                if (word.getTile()[i] != null && gameBoard[word.getRow() + i][word.getCol()].getTile() != null)
                    return false;
                if (word.getTile()[i] == null && gameBoard[word.getRow() + i][word.getCol()].getTile() == null)
                    return false;
            }
            ...
            for (int i = 0; i < word.getTile().length; i++) {
                if (word.getTile()[i] != null && gameBoard[word.getRow()][word.getCol() + i].getTile() != null)
                    return false;
                if (word.getTile()[i] == null && gameBoard[word.getRow()][word.getCol() + i].getTile() == null)
                    return false;
            }
    ...
    private Word fullWord(Word word) {
        ...
        for (int i = 0; i < len; i++) {
            if (word.getTile()[i] != null) {
                fullWordTiles[i] = word.getTile()[i];
            }
            else {
                if (word.isVertical()) {
